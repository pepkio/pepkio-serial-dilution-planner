...
from pepkio_serial_dilution_planner.config import TOOL_ID
from pepkio_serial_dilution_planner.exceptions import PepkioAPIError
from pepkio_serial_dilution_planner.models import RunResult
...
def test_get_manifest_and_cache(mock_manifest):
    ...
    with _make_client(handler) as client:
        ...
        assert m1["tool_id"] == "serial-dilution-planner"
        assert m1 is m2
    assert len(calls) == 1
...
def test_list_examples_and_get_example_input(mock_manifest):
    ...
    with _make_client(handler) as client:
        ...
        assert inp["num_steps"] == 4
        assert inp["stock_unit"] == "mM"
...
def test_run_sends_correct_request(mock_manifest, mock_run_response):
    ...
    with _make_client(handler) as client:
        result = client.run(client.get_example_input("standard_4step"), label="pytest")

    assert captured["path"] == f"/api/tools/v1/tools/{TOOL_ID}/run"
    assert captured["auth"] == "Bearer test-key"
    assert captured["body"]["input"]["num_steps"] == 4
...
def test_http_401_raises_api_error():
    ...
    with _make_client(handler) as client:
        with pytest.raises(PepkioAPIError) as exc:
            client.run({"stock_concentration": 1})
...
def test_run_response_error_field(mock_run_response):
    body = {**mock_run_response, "error": {"code": "TOOL_FAILED", "message": "bad dilution"}}

    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json=body)

    with _make_client(handler) as client:
        with pytest.raises(PepkioAPIError, match="bad dilution"):
            client.run({"num_steps": 1})
